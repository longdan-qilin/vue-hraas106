...
export default {
  data() {
    return {
      formData: '',
      printObj: {
        id: 'myPrint' // id表示要打印的那块区域
      },
      userId: this.$route.params.id, // 获取路由对象参数id
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getPersonalDetail(this.userId) // 获取个人详情
    },
    async getJobDetail() {
      this.userInfo = await getUserDetailById(this.userId)
      this.formData = await getJobDetail(this.userId)
      this.formData = { ...this.userInfo, ...this.formData } // 个人基本信息和个人岗位信息合并
    }
  }
}
</script>
