...
export default {
  ...
  created() {
    this.getUserDetailById() // 调用员工基本信息方法
    this.getPersonalDetail() // 调用员工个人信息方法
  },
  methods: {
    // 获取员工基本信息的方法
    async getUserDetailById() {
      ...
      // 如果员工基本信息有员工头像的话 赋值给fileList这个子组件的数组
    ...
    async saveUser() {
      try {
        ...
        // 判断上传的文件是否上传完成再去结构赋值数据
        if (fileList.some(item => !item.upload)) {
          // 此时upload为false 说明图片还在上传过程中
          // 如果此时去找 upload为false的图片 找到了说明 有图片还没有上传完成
          this.$message.warning('您当前还有图片还没有上传完成')
          return
        }
